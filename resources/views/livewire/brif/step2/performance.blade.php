<div
    ...
    <div class="card-body p-8 md:p-10">
        ...
        <form wire:submit='save' class="space-y-8" wire:ignore.self>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        По каким городам/странам нужно продвижение <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                ...
                <input wire:model.live='form.geography' type="text" placeholder="Москва, Санкт-Петербург, Россия"
                    class="input input-bordered w-full border-[#D0D0D0] focus:border-[#1A1A1A] bg-white h-11 md:h-12 px-4 text-[#1A1A1A] placeholder:text-[#9A9A9A] rounded-none text-sm
                        @error('form.geography') border-[#8B0000] @enderror" />
            </div>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Какие каналы продвижения вас интересуют
                        <span class="label-text text-xs text-[#6B6B6B] italic">
        ...
        'Контент-поддержка' => 'Контент-поддержка',
        'Веб-аналитика' => 'Веб-аналитика',
                        ...
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white
                                    @error('form.channels') border-[#8B0000] @enderror">
                            <input wire:model.live='form.channels' type="checkbox" value="{{ $value }}"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Что для вас важнее всего <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>

                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
                    @foreach ([
        'Увеличение трафика' => 'Увеличение трафика',
        'Рост продаж/лидов' => 'Рост продаж/лидов',
        'Управление репутацией (SERM)' => 'SERM',
        'Улучшение имиджа/репутации' => 'Улучшение имиджа/репутации',
        'Качественная аналитика и прозрачность' => 'Качественная аналитика и прозрачность',
    ] as $value => $description)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white
                                    @error('form.priorities') border-[#8B0000] @enderror">
                            <input wire:model.live='form.priorities' type="checkbox" value="{{ $value }}"
                                class="checkbox checkbox-sm border-[#D0D0D0] [--chkbg:#1A1A1A] [--chkfg:white] rounded-none" />
                            <div class="ml-2 md:ml-3">
                                <div class="font-medium text-[#1A1A1A] text-xs md:text-sm">{{ $value }}</div>
                                <div class="text-[10px] md:text-xs text-[#6B6B6B] mt-0.5">{{ $description }}</div>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Общий бюджет на перформанс-маркетинг в месяц <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
