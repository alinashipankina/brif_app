        ...
        <form wire:submit='save' class="space-y-8" wire:ignore.self>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        По каким городам/странам нужно показывать рекламу <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Запускали ли вы ранее контекстную рекламу <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.has_context_experience')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-2 gap-2 md:grid-cols-2 md:gap-3">
                    @foreach (['Да', 'Нет'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.has_context_experience') border-[#8B0000] @enderror">
                            <input wire:model.live='form.has_context_experience' type="radio"
                                value="{{ $value }}" name="has_context_experience"
            ...
            <div class="form-control">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
                    @foreach ([
        'Продажи/лиды' => 'Продажи/лиды',
        'Трафик на сайт' => 'Трафик на сайт',
        'Узнаваемость бренда' => 'Узнаваемость бренда',
                        ...
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white
                                    @error('form.campaign_goals') border-[#8B0000] @enderror">
                            <input wire:model.live='form.campaign_goals' type="checkbox" value="{{ $value }}"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Есть ли сезонность в бизнесе <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.has_seasonality')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-2 gap-2 md:grid-cols-2 md:gap-3">
                    @foreach (['Да', 'Нет', 'Не уверен(а)'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.has_seasonality') border-[#8B0000] @enderror">
                            <input wire:model.live='form.has_seasonality' type="radio" value="{{ $value }}"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Бюджет на контекстную рекламу в месяц
                    </span>
                    <span class="label-text text-xs text-[#6B6B6B] italic">
