<div
    ...
    <div class="card-body p-8 md:p-10">
        ...
        <form wire:submit='save' class="space-y-8" wire:ignore.self>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Какие системы аналитики уже установлены
                    </span>
                    <span class="label-text text-xs text-[#6B6B6B] italic">
        ...
        'Roistat' => 'Roistat',
        'Другое' => 'Другое',
        'Ничего не установлено' => 'Ничего не установлено',
                        ...
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white
                                    @error('form.analytics_systems') border-[#8B0000] @enderror">
                            <input wire:model.live='form.analytics_systems' type="checkbox" value="{{ $value }}"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Какие цели хотите отслеживать
                    </span>
                    <span class="label-text text-xs text-[#6B6B6B] italic">
                        *звонки, заявки, покупки и др.
                    </span>
                </label>
                @error('form.tracking_goals')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <input wire:model.live='form.tracking_goals' type="text" placeholder="Звонки"
                    class="input input-bordered w-full border-[#D0D0D0] focus:border-[#1A1A1A] bg-white h-11 md:h-12 px-4 text-[#1A1A1A] placeholder:text-[#9A9A9A] rounded-none text-sm
                        @error('form.tracking_goals') border-[#8B0000] @enderror" />
            </div>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Настроена ли у вас электронная коммерция/цели <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.ecommerce_setup')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-1 gap-2 md:grid-cols-3 md:gap-3">
                    @foreach (['Да', 'Нет', 'Частично'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.ecommerce_setup') border-[#8B0000] @enderror">
                            <input wire:model.live='form.ecommerce_setup' type="radio" value="{{ $value }}"
                                name="ecommerce_setup"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Есть ли доступ к рекламным кабинетам
                    </span>
                    <span class="label-text text-xs text-[#6B6B6B] italic">
                        *Яндекс Директ, VK Реклама
                    </span>
                </label>
                @error('form.has_ad_access')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-1 gap-2 md:grid-cols-2 md:gap-3">
                    @foreach (['Да', 'Нет'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.has_ad_access') border-[#8B0000] @enderror">
                            <input wire:model.live='form.has_ad_access' type="radio" value="{{ $value }}"
                                name="has_ad_access"
                                class="radio radio-xs border-[#D0D0D0] checked:bg-[#1A1A1A] checked:border-[#1A1A1A] bg-white" />
                            <span class="ml-2 md:ml-3 text-xs md:text-sm text-[#1A1A1A]">{{ $value }}</span>
                        </label>
        ...
        'Отчеты в Telegram/почту' => 'Отчеты в Telegram/почту',
        'Дашборды в Tableau/DataLens' => 'Дашборды в Tableau/DataLens',
        'Другое' => 'Другое',
    ] as $value => $description)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white
                                    @error('form.desired_outcomes') border-[#8B0000] @enderror">
                            <input wire:model.live='form.desired_outcomes' type="checkbox"
                                value="{{ $value }}"
                                class="checkbox checkbox-sm border-[#D0D0D0] [--chkbg:#1A1A1A] [--chkfg:white] rounded-none" />
                            <div class="ml-2 md:ml-3">
                                <div class="font-medium text-[#1A1A1A] text-xs md:text-sm">{{ $value }}</div>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Был ли опыт веб-аналитики ранее <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Бюджет на настройку и ведение аналитики
                    </span>
