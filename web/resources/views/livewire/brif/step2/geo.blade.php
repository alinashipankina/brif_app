        ...
        <form wire:submit='save' class="space-y-8" wire:ignore.self>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Какие вопросы чаще всего задают ваши клиенты? <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.client_questions')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <input wire:model.live='form.client_questions' type="text" placeholder="Опишите типичные вопросы"
                    class="input input-bordered w-full border-[#D0D0D0] focus:border-[#1A1A1A] bg-white h-11 md:h-12 px-4 text-[#1A1A1A] placeholder:text-[#9A9A9A] rounded-none text-sm
                        @error('form.client_questions') border-[#8B0000] @enderror" />
            </div>

            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Есть экспертный контент (статьи, исследования, кейсы)? <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.has_expert_content')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-1 gap-2 md:grid-cols-3 md:gap-3">
                    @foreach (['Да', 'Нет', 'В разработке'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.has_expert_content') border-[#8B0000] @enderror">
                            <input wire:model.live='form.has_expert_content' type="radio" value="{{ $value }}"
                                name="has_expert_content"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Нужен контент специально для GEO-продвижения <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.needs_geo_content')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-1 gap-2 md:grid-cols-3 md:gap-3">
                    @foreach (['Да', 'Нет', 'Не уверен(а)'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.needs_geo_content') border-[#8B0000] @enderror">
                            <input wire:model.live='form.needs_geo_content' type="radio"
                                value="{{ $value }}" name="needs_geo_content"
                                class="radio radio-xs border-[#D0D0D0] checked:bg-[#1A1A1A] checked:border-[#1A1A1A] bg-white" />
                            <span class="ml-2 md:ml-3 text-xs md:text-sm text-[#1A1A1A]">{{ $value }}</span>
                        </label>
                    @endforeach
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Был ли опыт GEO-продвижения ранее <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Комфортная сумма расходов на GEO-продвижение в месяц <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
