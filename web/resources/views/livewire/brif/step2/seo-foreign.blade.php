    ...
    <div class="card-body p-8 md:p-10">
        ...
        <form wire:submit='save' class="space-y-8" wire:ignore.self>
            <div class="form-control">
                <div class="flex flex-col md:flex-row md:items-baseline md:gap-2 mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Ссылка на сайт, который нужно продвигать
                    </span>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Как давно компания на рынке <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.year')
                    <label class="label">
                ...
                <div class="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-3">
                    @foreach (['Менее года', '1-3 года', '3-5 лет', 'Более 5 лет'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.year') border-[#8B0000] @enderror">
                            <input wire:model.live='form.year' type="radio" value="{{ $value }}" name="year"
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Страны для продвижения <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.countries')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <input wire:model.live='form.countries' type="text" placeholder="США, Великобритания, Германия и др."
                    class="input input-bordered w-full border-[#D0D0D0] focus:border-[#1A1A1A] bg-white h-11 md:h-12 px-4 text-[#1A1A1A] placeholder:text-[#9A9A9A] rounded-none text-sm
                        @error('form.countries') border-[#8B0000] @enderror" />
            </div>
            ...
            <div class="form-control">
                ...
                <input wire:model.live='form.languages' type="text"
                    placeholder="Английский, Немецкий, Французский и др."
                    class="input input-bordered w-full border-[#D0D0D0] focus:border-[#1A1A1A] bg-white h-11 md:h-12 px-4 text-[#1A1A1A] placeholder:text-[#9A9A9A] rounded-none text-sm
                        @error('form.languages') border-[#8B0000] @enderror" />
            </div>

            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Есть локализованная версия сайта <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                @error('form.has_localized')
                    <label class="label">
                        <span class="label-text-alt text-[#8B0000] text-xs mt-1">{{ $message }}</span>
                    </label>
                @enderror
                <div class="grid grid-cols-1 gap-2 md:grid-cols-3 md:gap-3">
                    @foreach (['Да, уже есть', 'Нет, требуется разработка', 'Частично'] as $value)
                        <label
                            class="flex items-center p-3 md:p-4 border border-[#D0D0D0] hover:border-[#1A1A1A] cursor-pointer transition-colors duration-200 bg-white @error('form.has_localized') border-[#8B0000] @enderror">
                            <input wire:model.live='form.has_localized' type="radio" value="{{ $value }}"
                                name="has_localized"
                                class="radio radio-xs border-[#D0D0D0] checked:bg-[#1A1A1A] checked:border-[#1A1A1A] bg-white" />
                            <span class="ml-2 md:ml-3 text-xs md:text-sm text-[#1A1A1A]">{{ $value }}</span>
                        </label>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider">
                        Был ли опыт продвижения ранее <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
            ...
            <div class="form-control">
                <label class="label mb-1.5 md:mb-2">
                    <span
                        class="label-text font-medium text-[#1A1A1A] text-xs md:text-sm uppercase tracking-wider break-words">
                        Комфортная сумма расходов на SEO-продвижение в месяц <span class="text-[#6B6B6B]">*</span>
                    </span>
                </label>
                ...
                <div class="space-y-2">
                    @foreach ([
        'от 40к до 100к рублей' => 'от 40 000 до 100 000 ₽',
        'от 100к до 250к рублей' => 'от 100 000 до 250 000 ₽',
        'от 250к до 500к рублей' => 'от 250 000 до 500 000 ₽',
        'от 500к до 1 млн рублей' => 'от 500 000 до 1 000 000 ₽',
        'от 1 млн рублей' => 'от 1 000 000 ₽',
    ] as $value => $label)
                        <label
